...
public class CommunicationWithJS {
	...
	public void isValid(Floor[] floors) {
		
		// verifico che ci sia almeno un marker per piano
		// e che, se ci sono due marker ci sia almeno una path
		
		boolean test = true;
		
		for (Floor f : floors) {
			test = test && (f.markers.size() > 0);
			
			if (f.markers.size() > 1)
				test = test && (f.paths.size() > 0);
			
			for (Map.Entry<Integer, Marker> m : f.markers.entrySet()) 
				test = test && (m.getValue().validated || m.getValue().valido);
				
			for (Path p : f.paths)
				test = test && p.validated;
		}
	...
	public String getPaths(Floor[] floors) {
		
		String response = "";
		
		for (Floor f : floors)
			response += f.paths.toString();
